            ...
            if sector['layer'] == layer:
                ...
                if sector['flags'][0] & 128 == 0:
                    ...
                    for v in sector['vertices']:
                        f_verts.append(new_bmesh.verts.new((v[0], v[1], sector['floor_alt'])))
                    ...
                    for w in sector['walls']:
                        ...
                        # some levels have duplicate walls 
                        try:
                            f_edges.append(new_bmesh.edges.new([f_verts[left], f_verts[right]]))
                        except:
                            print("Abnormal wall at sector ", sector['number'])

                    floor = bmesh.ops.triangle_fill(new_bmesh, edges=f_edges, use_dissolve=True, normal=(0.0, 0.0, 1.0))

                # ceiling (don't create if sky)
                if sector['flags'][0] & 1 == 0:
                    c_verts = []
                    for v in sector['vertices']:
                        c_verts.append(new_bmesh.verts.new((v[0], v[1], sector['ceiling_alt'])))
                    ...
                    for w in sector['walls']:
                        ...
                        # some levels have duplicate walls
                        try:
                            c_edges.append(new_bmesh.edges.new([c_verts[left], c_verts[right]]))
                        except:
                            pass    

                    ceil = bmesh.ops.triangle_fill(new_bmesh, edges=c_edges, use_dissolve=True, normal=(0.0, 0.0, -1.0))
...
class ImportLev(Operator, ImportHelper):
    """Import a DF Level"""
    bl_idname = "import_vue.dflev"  
...
def menu_func_import(self, context):
    self.layout.operator(ImportLev.bl_idname, text="Dark Forces Level (.LEV)")
